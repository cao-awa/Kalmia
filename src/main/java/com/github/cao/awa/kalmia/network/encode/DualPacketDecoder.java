...
public class DualPacketDecoder extends ByteToMessageDecoder {
    ...
    public DualPacketDecoder(RequestRouter router) {
        this.router = router;
    }
    ...
    private void done(byte[] data, List<Object> out) {
        ...
        byte[] receipt = reader.read() == - 1 ? Request.RECEIPT : ReceiptRequest.read(reader);
