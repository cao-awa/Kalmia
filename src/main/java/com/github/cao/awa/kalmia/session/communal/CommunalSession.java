...
public class CommunalSession extends GroupSession {
    ...
    public static final CommunalSession TEST_COMMUNAL = new CommunalSession(TEST_COMMUNAL_IDENTITY, "Test public session", 0);
    ...
    public CommunalSession(PureExtraIdentity sessionIdentity, String displayName, long subscriberCount) {
        super(sessionIdentity, displayName, subscriberCount);
    }

    public static CommunalSession create(BytesReader reader) {
        if (reader.read() == 2) {
            return new CommunalSession(PureExtraIdentity.read(reader), new String(reader.read(Base256.readTag(reader)), StandardCharsets.UTF_8), SkippedBase256.readLong(reader));
        }
        return null;
    }
    ...
    public boolean accessible(LongAndExtraIdentity accessIdentity) {
        return Kalmia.SERVER.getSessionManager().accessible(identity(), accessIdentity).accessibleChat(false);
    }
